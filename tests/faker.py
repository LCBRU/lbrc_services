...
class TaskCreator(FakeCreator):
    ...
    def _create_item(self, save: bool, args: FakeCreatorArgs):
        ...
        requestor = args.get('requestor', lambda: self.faker.user().get(save=save))
...
class QuoteStatusCreator(FakeCreator):
    ...
    def _create_item(self, save: bool, args: FakeCreatorArgs):
        quote = args.get('quote', lambda: self.faker.quote().get(save=save))
...
class QuoteWorkLineCreator(FakeCreator):
    ...
    def _create_item(self, save: bool, args: FakeCreatorArgs):
        quote_work_section = args.get('quote_work_section', lambda: self.faker.quote_work_section().get(save=save))
        name = args.get('name', self.faker.sentence(nb_words=randint(1,10)))
...
class TaskFileCreator(FakeCreator):
    ...
    def _create_item(self, save: bool, args: FakeCreatorArgs):
        ...
        task = args.get('task', lambda: self.faker.task().get(save=save))
        field = args.get('field', lambda: self.faker.field().get(save=save))
