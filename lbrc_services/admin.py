...
class ServiceView(AdminCustomView):
    ...
    form_args = dict(
        ...
        field_group=dict(query_factory=lambda: db.session.scalars((select(FieldGroup).order_by(FieldGroup.name)).unique())),
        owners=dict(query_factory=lambda: db.session.scalars((select(User).order_by(User.last_name, User.first_name, User.email)).unique())),
        excluded_organisations=dict(query_factory=lambda: db.session.scalars((select(Organisation).order_by(Organisation.name)).unique())),
    )
    column_list = [
        Service.name,
        "field_group",
